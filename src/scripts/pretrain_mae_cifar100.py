...
def get_args():
    ...
    # Dataset parameters
    parser.add_argument('--data_path', type=str, default='data/cifar100', help='path to ImageNet data')
...
def main(args):
    ...
    # Dataset
    dataset_train = CIFAR100(path=args.data_path, train=True, transform=transform_train)
    ...
    for epoch in range(args.epochs):
        ...
        lr_scheduler.step()
        if (epoch + 1) % 50 == 0:
            torch.save(mae.state_dict(), f'{args.output_dir}/pretrain_mae_cifar100_epoch{epoch + 1}.pth')
            end_time = time.time()
